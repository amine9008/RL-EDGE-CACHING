...
def main(episode_start=0, nb_vehicules = 6, nb_rsus = 3, nb_content = 3):
    ...
    print("[Python] DMRE Training Start...")
    agents = {}
    ...
    print("[Python] Number of agents (N-I) {}".format(len(agents)))
    episode_rewards = []
    episode_flops = []
    episode_hits = []
    episode_misses = []
        ...
        while not done:
            ...
            for i in range(N):
                if i < I: # RSU
                    ...
                    action[N-I+i] = 1 # took caching decision, as stated by authors of ECSMADRL, there is always a caching
                    action[2*N-I+i] = F-1 # took the replacement decision, as stated by authors, a least popular replacement is taken, thus replacing the content F-1
                else: # Vehicules
                    ...
                    action[2*N-I+i] = F-1 # the replacement will follow the authors policy, replacing the least popular content (F-1)
